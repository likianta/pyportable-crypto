...
def generate_cipher_package(
    ...
    if fs.exists(dir_m):
        ...
        dumps(dedent('''
            ...
            __version__ = '{1}'
        ''').format(pyversion, crypto_version), dir_o + '/__init__.py')
    
    fs.copy_file(fs.find_file_paths(dir_m, ('.pyd', '.so'))[0], file_o)
    fs.copy_file('{}/__init__.py'.format(dir_i), '{}/__init__.py'.format(dir_o))
